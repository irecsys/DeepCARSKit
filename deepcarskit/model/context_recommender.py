...
class ContextRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        for i in range(2, len(self.field_names)):
            if self.field_names[i] == config['LABEL_FIELD'] or self.field_names[i] == config['USER_CONTEXT_FIELD'] or self.field_names[i] == config['RATING_FIELD']:
                continue
            else:
                self.CONTEXTS.append(self.field_names[i])

        self.n_context_situation = 0
        if self.CONTEXT_SITUATION_ID in self.CONTEXTS:
            self.n_context_situation = dataset.num(self.CONTEXT_SITUATION_ID)
            self.CONTEXTS.remove(self.CONTEXT_SITUATION_ID)

        msghead = "Loaded context variables: "
        if self.n_context_situation == 0:
            msg = ' '.join(self.CONTEXTS) + ', without context situation ID: ' + self.CONTEXT_SITUATION_ID
        else:
            msg = ' '.join(self.CONTEXTS) + ', with context situation ID: ' + self.CONTEXT_SITUATION_ID
        self.logger.info(set_color(msghead, 'yellow') + msg)

        self.n_users = dataset.num(self.USER_ID)
        self.n_items = dataset.num(self.ITEM_ID)
        # number of context variables
        self.n_contexts_dim = len(self.CONTEXTS)
        # number of context conditions in each dimension
        self.n_contexts_conditions = []

        for i in range(self.n_contexts_dim):
            dim=self.CONTEXTS[i]
            n_dim = dataset.num(dim)
            self.n_contexts_conditions.append(n_dim)
        ...
        for field_name in self.field_names:
            if field_name == self.config['RATING_FIELD'] or field_name == self.config['LABEL_FIELD']:
                continue
            if dataset.field2type[field_name] == FeatureType.TOKEN:
    ...
    def getContextSituationList(self, interaction, context_dims):
        situation = []
        for dim in context_dims:
            situation.append(interaction[dim].tolist())
        situation = torch.tensor(situation).to(self.device)
        return situation

    def getContextSituationDict(self, interaction, context_dims):
        situation = {}
        for dim in context_dims:
            situation[dim] = interaction[dim]
        return situation
