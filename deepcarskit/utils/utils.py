...
def get_model(model_name):
    ...
    model_submodule_recbole = [
        'general_recommender',  'sequential_recommender', 'knowledge_aware_recommender',
         'exlib_recommender'
    ]

    model_submodule_deepcarskit = [
        'ae', 'fms', 'neucf'
    ]

    model_file_name = model_name.lower()
    model_module = None
    for submodule in model_submodule_deepcarskit:
        module_path = '.'.join(['deepcarskit.model', submodule, model_file_name])
        if importlib.util.find_spec(module_path, __name__):
            model_module = importlib.import_module(module_path, __name__)
            break

    if model_module is None:
        for submodule in model_submodule_recbole:
...
def get_trainer(model_type, model_name):
    ...
    """
    try:
        return getattr(importlib.import_module('deepcarskit.trainer'), model_name + 'Trainer')
