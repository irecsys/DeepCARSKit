...
class Dataset(object):
    ...
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~deepcarskit.data.dataset.dataset.Dataset`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~deepcarskit.data.dataset.dataset.Dataset.build`, it will processing dataset into
    DataLoaders, according to :class:`~deepcarskit.config.eval_setting.EvalSetting`.
    ...
    Attributes:
        ...
        field2type (dict): Dict mapping feature name (str) to its type (:class:`~deepcarskit.utils.enum_type.FeatureType`).

        field2source (dict): Dict mapping feature name (str) to its source
            (:class:`~deepcarskit.utils.enum_type.FeatureSource`).
    ...
    def __init__(self, config):
        ...
        self._from_scratch()  # David Wang: get data in here

    def _get_innerid_from_rawid(self, field, rawid):
        innerid = -1
    ...
    def _get_uid_from_usercontexts(self, innerid_usercontexts):
        str_uc = self._get_rawid_from_innerid(self.config['USER_CONTEXT_FIELD'], innerid_usercontexts)
        list_strs = str_uc.split('_')
        uid = int(list_strs[0])
        return uid

    def _get_context_tuple_from_usercontexts(self, innerid_usercontexts):
        str_uc = self._get_rawid_from_innerid(self.config['USER_CONTEXT_FIELD'], innerid_usercontexts)
        list_strs = str_uc.split('_')
        list_integers = list(map(int, list_strs))
        del (list_integers[0])
        tuple_contexts = tuple(list_integers)
        return tuple_contexts

    def _get_context_fields(self):
        label = self.config['LABEL_FIELD']
        rating = self.config['RATING_FIELD']
        cols = list(self.inter_feat.columns)
        cols.remove(self.uid_field)
        cols.remove(self.iid_field)
        cols.remove(self.ucid_field)
        if label in cols:
            cols.remove(label)
        if rating in cols:
            cols.remove(rating)
    ...
    def _get_field_from_config(self):
        ...
        self.ucid_field = self.config['USER_CONTEXT_FIELD']
    ...
    def _data_processing(self):
        ...
        if self.config['eval_type'] == EvaluatorType.RANKING:
            ...
            # msghead='Task: Top-N Recommendations evaluated by top-N metrics'
        # else:
            # msghead='Task: Rating Predictions evaluated by value-based metrics'
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~deepcarskit.data.dataset.dataset.Dataset._reset_index` will reset the index of feats.
        """
    ...
    def _load_data(self, token, dataset_path):
        ...
        """
        # David Wang: load interaction data, user data, item data
        ...
        if self.benchmark_filename_list is None:
            ...
            for filename in self.benchmark_filename_list:
                ...
                if os.path.isfile(file_path):
                    # David Wang: save data into a temporary data frame
            ...
            # David Wang: save loaded data into inter_feat attribute
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        Note:
            ``user_id`` and ``item_id`` has source :obj:`~deepcarskit.utils.enum_type.FeatureSource.USER_ID` and
            :obj:`~deepcarskit.utils.enum_type.FeatureSource.ITEM_ID`
        """
    ...
    def _load_feat(self, filepath, source):
        ...
        # David Wang: identify columns and data type
        ...
        for field_type in head.split(field_separator):
            ...
            columns.append(field)   # David Wang: ['user_id', 'item_id', 'rating', 'timestamp']
            usecols.append(field_type)   # David Wang:  ['user_id:token', 'item_id:token', 'rating:float', 'timestamp:float']
        ...
        # David Wang: load data into data frame
        df = pd.read_csv(
        ...
        # David Wang: clear up data with NA or None value
    ...
    def _fill_nan(self):
        ...
        For fields with type :obj:`~deepcarskit.utils.enum_type.FeatureType.TOKEN`, missing value will be filled by
        ``[PAD]``, which indexed as 0.

        For fields with type :obj:`~deepcarskit.utils.enum_type.FeatureType.FLOAT`, missing value will be filled by
    ...
    def _get_remap_list(self, field_list):
        ...
        for field in field_list:
            ftype = self.field2type[field]
            for feat in self.field2feats(field):
                if field == self.ucid_field:
                    # update the values in the filed of context sitution
                    # raw value: nominal text values
                    # new value: mapped innerID
                    context_cols = []
                    context_cols.append(self.uid_field)
                    context_cols += list(feat.columns[3:])
                    context_cols.remove(self.ucid_field)
                    if self.config['LABEL_FIELD'] in context_cols:
                        context_cols.remove(self.config['LABEL_FIELD'])
                    # use new inner ID to replace the raw context conditions
                    feat[self.ucid_field] = feat[context_cols[0]].astype(str)
                    for i in range(1, len(context_cols)):
                        feat[self.ucid_field] = feat[self.ucid_field].str.cat(feat[context_cols[i]].astype(str), sep='_')
    ...
    def unique(self, field):
        values = numpy.unique(self.field2id_token[field])
        return values
    ...
    def float_like_fields(self):
        """Get fields of type :obj:`~deepcarskit.utils.enum_type.FeatureType.FLOAT` and
        :obj:`~deepcarskit.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def token_like_fields(self):
        """Get fields of type :obj:`~deepcarskit.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~deepcarskit.utils.enum_type.FeatureType.TOKEN_SEQ`.
    ...
    @property
    def seq_fields(self):
        """Get fields of type :obj:`~deepcarskit.utils.enum_type.FeatureType.TOKEN_SEQ` and
        :obj:`~deepcarskit.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def non_seq_fields(self):
        """Get fields of type :obj:`~deepcarskit.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~deepcarskit.utils.enum_type.FeatureType.FLOAT`.
    ...
    @property
    def user_context_num(self):
        self._check_field('ucid_field')
        return self.num(self.ucid_field)
    ...
    @property
    def avg_actions_of_user_context(self):
        if isinstance(self.inter_feat, pd.DataFrame):
            return np.mean(self.inter_feat.groupby(self.ucid_field).size())
        else:
            return np.mean(list(Counter(self.inter_feat[self.ucid_field].numpy()).values()))
    ...
    def __str__(self):
        ...
        info.append(set_color('Remain Fields', 'blue') + f': {fields}')

        if self.ucid_field in fields:
            fields.remove(self.ucid_field)
        if self.config['LABEL_FIELD'] in fields:
            fields.remove(self.config['LABEL_FIELD'])

        for i in range(3, len(fields)):
            dim = fields[i]
            if dim == self.config['CONTEXT_SITUATION_FIELD']:
                continue
            n_dim = self.num(dim)
            conditions = self.unique(dim)
            msghead = "Context dimension - "+dim+ ": "+ str(n_dim) + " values: "
            info.append(set_color(msghead, 'blue') + f': {conditions}')
    ...
    def _calcu_split_ids(self, tot, ratios):
        ...
        cnt[0] = tot - sum(cnt[1:])  # David Wang: make sure the sum = tot
        for i in range(1, len(ratios)):
            if cnt[0] <= 1:
                break
            if 0 < ratios[-i] * tot < 1:  # David Wang: make sure each part has at least one
                cnt[-i] += 1
                cnt[0] -= 1
    ...
    def split_by_folds(self, folds, group_by=None):
        ...
        self._drop_unused_col()
        ds_split_dict = {}
        for fold in folds_ind_dict:
            next_index = folds_ind_dict[fold]
            next_df = [self.inter_feat[index] for index in next_index]
            next_ds = [self.copy(_) for _ in next_df]
            ds_split_dict[fold] = next_ds
    ...
    def build(self):
        ...
        See :class:`~deepcarskit.config.eval_setting.EvalSetting` for details.
    ...
    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None):
        ...
        Args:
            ...
            source_field (str): Source field: user_id  (added by David Wang)
            target_field (str): Target field: movie_id (added by David Wang)
            form (str, optional): Sparse matrix format. Defaults to ``coo``.
            value_field (str, optional): Data of sparse matrix, which should exist in ``df_feat``.
                Defaults to ``None``. can take rating, rating1, rating2, rating3, rating4 (added by David Wang)
    ...
    def _dataframe_to_interaction(self, data):
        """Convert :class:`pandas.DataFrame` to :class:`~deepcarskit.data.interaction.Interaction`.
