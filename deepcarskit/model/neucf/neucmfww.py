...
class NeuCMFww(ContextRecommender):

    input_type = InputType.POINTWISE

    def __init__(self, config, dataset):
        super(NeuCMFww, self).__init__(config, dataset)
    ...
    def forward(self, user, item, context_situation):
        ...
        context_situation_mlp_e = self.context_situation_mlp_embedding(context_situation)
        if self.mf_train:
